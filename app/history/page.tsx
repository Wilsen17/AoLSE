...
export default function HistoryPage() {
  ...
  useEffect(() => {
    ...
    if (storedOrders) {
      try {
        ...
        groupOrdersByDate(parsedOrders)
      } catch (e) {
  ...
  const groupOrdersByDate = (orders: Order[]) => {
    // Create a map to group orders by date (just the date part, not time)
    ...
    orders.forEach((order) => {
      ...
      if (!orderMap.has(dateKey)) {
        orderMap.set(dateKey, {
          date: dateKey,
          dateFormatted: formatDate(order.date, order.id),
          orders: [],
          totalItems: 0,
          totalAmount: 0,
        })
      ...
      const group = orderMap.get(dateKey)!
    ...
    const groupedArray = Array.from(orderMap.values())
  ...
  const handleOrderAgain = (groupedOrder: GroupedOrder) => {
    // Check if user is logged in
    if (!user) {
      router.push("/login")
      return
    ...
    // Collect all items from all orders in this group
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {groupedOrders.map((groupedOrder) => (
                      <tr key={groupedOrder.date} className="border-b border-white/10 hover:bg-[#6a7c3f]">
                        <td className="py-4 px-4">
                          <p className="text-[#FFD700] font-bold">{groupedOrder.dateFormatted}</p>
                        </td>
                        <td className="py-4 px-4">
                            ...
                            {groupedOrder.orders.flatMap((order) =>
                              order.items.map((item, idx) => (
                                <div key={`${order.id}-${idx}`} className="flex items-center gap-3">
                                  ...
                                  <div className="bg-[#FFF9E2] rounded-lg p-2 inline-block">
                          ...
                          <div className="flex flex-col space-y-2 items-center">
                            {groupedOrder.orders.flatMap((order) =>
                              order.items.map((item, idx) => (
                                <div
                                  key={`${order.id}-${idx}`}
                                  className="w-10 h-6 bg-[#FFC300] rounded-full flex items-center justify-center"
                                >
                        ...
                        <td className="py-4 px-4 text-center">
                          <span className="text-[#FFD700] font-bold text-xl">
                            Rp. {groupedOrder.totalAmount.toLocaleString()}
                          </span>
                        </td>
                        <td className="py-4 px-4 text-center">
                          <Button
                            onClick={() => handleOrderAgain(groupedOrder)}
